...
Read this when **Patterns A-F in `auth-decision-tree.md` aren't viable**: the SaaS gates auth behind machinery the Bun sidecar can't run (real Chromium fingerprint + JS-computed cookies + SameSite-strict redirects across hosts). FlareSolverr — a Chromium-backed proxy — bypasses these by running an actual headless browser, then exposing its cookie jar + navigation API over HTTP.
...
|---|---|
| `error=Callback` (next-auth) at the end of an otherwise-clean OIDC flow. The login form POST succeeds, but the final `/api/auth/callback/<provider>?code=…&state=…` lands the user on an error page instead of a session. | next-auth verifies a `__Secure-next-auth.state` cookie (HttpOnly, SameSite=Lax) on the callback. Bun fetch in the sidecar drops SameSite=Lax cookies on cross-host redirects (`id.example.com → www.example.com`). Chromium handles them correctly. |
